...
public class ProdCalendar extends JPanel {
	...
	public ProdCalendar(MainFrame mainFrame, litheraa.data.calendar.Calendar data) {
		...
		monthLabel.setBackground(((ThemeColors) ViewController.getTheme()).getAccentBackground());
		monthLabel.setForeground(((ThemeColors) ViewController.getTheme()).getAccentForeground());

		JLabel fakeLayout = new JLabel();
		fakeLayout.setOpaque(true);
		fakeLayout.setBackground(((ThemeColors) ViewController.getTheme()).getAccentBackground());
		fakeLayout.add(monthLabel);
		add(fakeLayout);

		SpringLayout sL = new SpringLayout();
		sL.putConstraint(SpringLayout.HORIZONTAL_CENTER, monthLabel, 0, SpringLayout.HORIZONTAL_CENTER, fakeLayout);
		sL.putConstraint(SpringLayout.HEIGHT, monthLabel, 0, SpringLayout.HEIGHT, fakeLayout);
		fakeLayout.setLayout(sL);
...
///		there is some strange bug in util.Calendar getActualMaximum(week_of_month) for November and December months. So, quick solution
		...
		layout.putConstraint(SpringLayout.WIDTH, fakeLayout, 0, SpringLayout.WIDTH, this);
		layout.putConstraint(SpringLayout.HORIZONTAL_CENTER, fakeLayout, 0, SpringLayout.HORIZONTAL_CENTER, this);
		layout.putConstraint(SpringLayout.NORTH, fakeLayout, 0, SpringLayout.NORTH, this);
		layout.putConstraint(SpringLayout.SOUTH, fakeLayout, 35, SpringLayout.NORTH, this);
		layout.putConstraint(SpringLayout.NORTH, dayNamesPanel, 1, SpringLayout.SOUTH, fakeLayout);
		layout.putConstraint(SpringLayout.SOUTH, dayNamesPanel, 25, SpringLayout.SOUTH, fakeLayout);
